...
export default class InputBuilder {
  ...
  public async getUserInputs(): Promise<{
    ...
    try {
      ...
      this.modulePluralName = trimSlashes(
        await this.readModulePluralName()
      ).trim();
  ...
  private async readModuleName(): Promise<string> {
    return input({
      ...
      validate: (input: string) =>
        input ? true : "Module name cannot be empty.",
    });
  }
  ...
  private async readModuleFriendlyName(): Promise<string> {
    return input({
      ...
      validate: (input: string) =>
        input ? true : "Friendly name cannot be empty.",
    });
  }
  ...
  private async readModuleSingularName(): Promise<string> {
    return input({
      ...
      validate: (input: string) =>
        input ? true : "Module singular name cannot be empty.",
  ...
  private async readModulePluralName(): Promise<string> {
    return input({
      ...
      validate: (input: string) =>
        trimSlashes(input.trim())
          ? true
          : "Module plural name cannot be empty.",
  ...
  private async readLanguageMode(): Promise<LanguageMode> {
    return (await input({
      message:
        "Choose the language for the module (1 for JavaScript (soon), 2 for TypeScript):",
      default: "2",
      validate: (input: string) =>
        ["2", "1"].includes(input) ? true : "Please select a valid option.",
    })) === "2"
      ? "typescript"
      : "javascript";
  }
  ...
  private async readReducerName(): Promise<string> {
    return input({
      ...
      validate: (input: string) =>
        input ? true : "Reducer name cannot be empty.",
    });
  }
  ...
  private async readBaseRouteName(): Promise<string> {
    return input({
      ...
      validate: (input: string) =>
        input ? true : "You must enter the base route name.",
    });
  }
  ...
  private async readBaseRouteId(): Promise<string> {
    return input({
      ...
      default: "id",
      validate: (input: string) =>
        input ? true : "You must enter the base route id.",
    });
  }
}
